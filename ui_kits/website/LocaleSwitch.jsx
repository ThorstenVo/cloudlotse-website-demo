...
function LocaleSwitch({ locale, compact = false }) {
  ...
  return (
    <span style={{ display: "inline-flex", border: "1px solid rgb(255 255 255 / 35%)" }} aria-label={window.EazyCloudI18n.TRANSLATIONS[locale].a11y.language}>
      {locale === "de"
        ? <><span aria-current="page" style={activeStyle}>DE</span><a href={href} onClick={switchLanguage} style={linkStyle}>EN</a></>
        : <><a href={href} onClick={switchLanguage} style={linkStyle}>DE</a><span aria-current="page" style={activeStyle}>EN</span></>}
    </span>
  );
}
